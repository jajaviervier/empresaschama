...
function cargarCierre() {
    ...
    db.ref(rutaDatosImportantesCuadratura).once('value', function(iCuadratura) {
        ...
        cajabase2 = cajabase
        if (ganancia > 0) {
            console.log("modificando ganancia");
            cajaBasePorRecuperar2 = cajaBasePorRecuperar - ganancia;
            cajaBasePorRecuperar3 = cajaBasePorRecuperar2;
            if (cajaBasePorRecuperar2 < 0) {
                cajaBasePorRecuperar2 = 0;
            }
            ganancia -= cajaBasePorRecuperar * -1;
            cajabase = cajabase + (cajaBasePorRecuperar * -1);
        } else {
            console.log("no se necesita modificar ganancia");
        }
        if (ganancia > 0) {
        ...
        console.log(parseInt(cajaBaseReferenciall));
        console.log("caja base referencial ")
        diferencia = montoEntregaDeCaja - (ganancia + cajabase)
        deposito = ganancia - (diferencia * -1);
        $('#cajaBaseReferencial').html(puntuar(cajaBaseReferenciall));
        $('#entregaDos').html(puntuar(montoEntregaDeCaja))
